                            ...
                            <div class="dt-responsive p-4" style="overflow: scroll;">
                                <table class=" table table-bordered display nowrap" id="alt-pg-dt" style="font-size: 16px;">
                                    ...
                                    <tbody>
                                        <?php
                                        $no = 1;
                                        $sql = mysqli_query($konek, "SELECT * FROM penarikan INNER JOIN anggota USING(ID_Tabungan) WHERE Status_Penarikan='Konfirmasi' GROUP BY ID_Anggota");
                                        while ($p = mysqli_fetch_array($sql)) {
                                            ...
                                            $sql_penarikan   = mysqli_query($konek, "SELECT SUM(Besar_Penarikan) AS penarikan FROM penarikan 
                                                        INNER JOIN anggota ON anggota.ID_Tabungan = penarikan.ID_Tabungan WHERE Status_Penarikan='Konfirmasi' AND ID_Anggota = '$p[ID_Anggota]'");
                                        ...
                                        ?>
                                            <tr style="<?= $color; ?>">
                                                ...
                                                <td align="right"><?= rupiah($total_penarikan); ?></td>
                                                <td align="center"><?= $p["Status_Penarikan"]; ?></td>
                                                <td align="center"><a href="detail_penarikan.php?ID_Anggota=<?= $p['ID_Anggota'] ?>" title="Detail Penarikan <?= $p['Nama_Anggota']; ?>"><button class="btn btn-icon btn-outline-primary"><i class='fas fa-eye'></i></button></a></td>
